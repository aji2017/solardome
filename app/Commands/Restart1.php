...
class Restart1 extends BaseCommand
{
    protected $group       = 'esp';
    protected $name        = 'esp:restart';
    protected $description = 'Looping aktif kontrol Restart ESP via MQTT';
    ...
    public function __construct(LoggerInterface $logger, Commands $commands)
    {
        ...
        $this->statusFile = WRITEPATH . 'restart_esp.json';
    }
    ...
    public function run(array $params)
    {
        ...
        $port     = 8883;
        $clientId = 'solardome_restart_esp-' . uniqid();
        $username = 'admin';
        $password = 'admin';
        ...
        echo "🚀 Monitoring aktif untuk restart_esp.json...\n";

        while (true) {
            try {
                if (!file_exists($this->statusFile)) {
                    echo "⚠️  File tidak ditemukan, membuat file baru...\n";
                    file_put_contents($this->statusFile, json_encode(['esp' => 'false']));
                }

                $currentState = json_decode(file_get_contents($this->statusFile), true);

                if (!is_array($currentState) || !isset($currentState['esp'])) {
                    echo "❌ Format JSON tidak valid. Reset file...\n";
                    file_put_contents($this->statusFile, json_encode(['esp' => 'false']));
                    continue;
                }

                if ($currentState['esp'] === 'true') {
                    $topic = 'solardome/cntrl/restart/esp';
